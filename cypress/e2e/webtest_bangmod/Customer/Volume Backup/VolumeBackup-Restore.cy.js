...
describe('Volume Backup / Manage / Restore', () => {
    beforeEach(() => {
    ...
    it('Usabilities (User click Restore button. The system display modal :)', () => {
        cy.get('tbody tr').eq(0)
            ...
            .click();
        cy.wait(200)
        cy.get('.btn').contains('Restore').click();
        ...
        cy.readFile(csvFilePath).then(csvData => {
            ...
            data.forEach((row) => {
                const textmodal = row.textModalRestore;

                cy.get('.ant-modal-content').within(() => {
                    cy.contains('.ant-modal-title', 'Restore Backup to New Volume?')
                        .should('be.visible')
                        .and('contain', 'Restore Backup to New Volume?')
                        .get('.ant-modal-body')
                        .contains(textmodal)
                        .wait(300)
